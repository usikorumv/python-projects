# import subprocess
# data = subprocess.check_output(['netsh', 'wlan', 'show', 'profiles']).decode('CP866').split('\n')
# profiles = [i.split(":")[1][1:-1] for i in data if "Все профили пользователей" in i]
# for profile in profiles:
#   results = subprocess.check_output(['netsh', 'wlan', 'show', 'profile', profile, 'key=clear']).decode('CP866').split('\n')
#   result = [b.split(":")[1][1:-1] for b in results if "Содержимое ключа" in b]
#   try:
#     print ("{:<30}| {:<}".format(profile, result[0]))
#   except IndexError:
#     print ("{:<30}| {:<}".format(profile, ""))
